...
test('real clients match, join private rooms, receive identical authoritative grids and leave', {skip:!enabled,timeout:30000},async()=>{
 ...
 try{
  const a=await join('joinOrCreate','arena',{map:'square'});const sa=await snapshot(a);a.onMessage('snapshot',()=>{});
  const b=await join('joinOrCreate','arena',{map:'square'});const sb=await snapshot(b);b.onMessage('snapshot',()=>{});assert.equal(a.roomId,b.roomId);assert.notEqual(sa.playerId,sb.playerId);assert.equal(sb.entities.filter(e=>e.human).length,2);assert.equal(sb.entities.filter(e=>!e.human).length,16);
  a.send('aim',0);a.send('aim',NaN);a.send('aim',{x:999999});
  const privateRoom=await join('create','arena',{map:'taiwan',private:true});await snapshot(privateRoom);privateRoom.onMessage('snapshot',()=>{});
  const friend=await join('joinById',privateRoom.roomId,{map:'usa'});const sf=await snapshot(friend);friend.onMessage('snapshot',()=>{});assert.equal(sf.map,'taiwan');
  const stranger=await join('joinOrCreate','arena',{map:'taiwan'});await snapshot(stranger);stranger.onMessage('snapshot',()=>{});assert.notEqual(stranger.roomId,privateRoom.roomId);
